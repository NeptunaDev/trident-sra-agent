...
export default function TypescriptTable({ rows = [], onViewLog, onDelete, t }) {
  ...
  return (
    <div className="bg-[#11111f] border border-[rgba(91,194,231,0.2)] rounded-lg overflow-hidden">
      <h3 className="px-4 py-3 border-b border-[rgba(91,194,231,0.2)] text-white font-semibold">{t('media.typescriptsTitle')}</h3>
            ...
            return (
              <tr key={session.sessionId} className="border-b border-neptuna/8 hover:bg-[#1a1a2e] transition-colors">
                ...
                <td className="py-3 px-4">
                  <MediaActions
                    t={t}
                    onView={() => onViewLog(session.sessionId)}
                    onViewLabel={t('media.viewLog')}
      ...
      {rows.length === 0 && <p className="p-4 text-sm text-[#9ca3af]">{t('media.emptyTypescripts')}</p>}
      {rows.length > 0 && (
        <Pagination
          page={safePage}
