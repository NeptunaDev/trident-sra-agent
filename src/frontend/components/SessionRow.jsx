...
import { formatSessionDate, shortenSessionId, cellWithTitle } from '../utils/format';

export default function SessionRow({ session, onViewVideo, onViewLog, onDeleteSession }) {
    ...
    >
      ...
      <td className="py-3 px-4">
        <TooltipAnchor text={videoInfo.title} className="cursor-default">
          <span className="text-[#c0c5ce]">{videoInfo.short === '—' ? '—' : 'Sí'}</span>
        </TooltipAnchor>
      </td>
      <td className="py-3 px-4">
        <TooltipAnchor text={typescriptInfo.title} className="cursor-default">
          <span className="text-[#c0c5ce]">{typescriptInfo.short === '—' ? '—' : 'Sí'}</span>
        </TooltipAnchor>
      </td>
      <td className="py-3 px-4">
